...
public:
    ...
    int viability;
    std::unordered_map<int, double> chromosome_length;
...
void repaired_chromosome::build_graph_and_classify() {
    ...
    // Helper: does a segment between two ends contain a centromere?
    auto segment_has_centromere = [](const dsb_end& a, const dsb_end& b) {
        // Rule 1: different arms → must cross centromere
        if (a.chromosome_arm != b.chromosome_arm) return true;

        // Same arm:
        // p-arm (0): centromere is "downstream"
        // q-arm (1): centromere is "upstream"
        // Opposite orientation → crosses centromere
        if (a.upstream_downstream != b.upstream_downstream) return true;

        return false;
    };

    // Group repair pairs by chromosome
    unordered_map<int, vector<const dna_repair_pair*>> by_chrom;
    for (const auto& rp : repair_pairs) {
        int c1 = rp.end1.chromosome_id;
        int c2 = rp.end2.chromosome_id;
        if (c1 == c2) {
            by_chrom[c1].push_back(&rp);
        } else {
            // Inter-chromosome misrepair → always linear, never a ring
            by_chrom[c1].push_back(&rp);
            by_chrom[c2].push_back(&rp);
        }
    }

    int total_rings = 0;
    int total_dicentrics = 0;
    int total_excess_linear = 0;
    int total_multicentrics = 0;
    ...
    for (auto& kv : by_chrom) {
        ...
        for (auto* rp : pairs) {
            ends.insert(rp->end1.break_index);
            ends.insert(rp->end2.break_index);
        }

        int num_pairs = pairs.size();
        int num_ends = ends.size();

        bool is_ring = (num_pairs == num_ends);

        // Count centromeres
        int centromere_count = 0;
        for (auto* rp : pairs) {
            if (segment_has_centromere(rp->end1, rp->end2)) {
                centromere_count++;
            }
        }
        ...
        if (is_ring) {
            ...
            if (centromere_count >= 2) {
                total_dicentrics++;
                if (centromere_count > 2) total_multicentrics++;
            }
        } else {
    ...
    after_reconstruction_dicentrics = total_dicentrics;
    after_reconstruction_rings = total_rings;
    after_reconstruction_excess_linear = total_excess_linear;
    after_reconstruction_multicentrics = total_multicentrics;
    after_reconstruction_total_aberration =
        total_dicentrics + total_rings + total_excess_linear + total_multicentrics;
}

void repaired_chromosome::visualize_segments() {
    using namespace std;

    // Helper: does a segment between two ends contain a centromere?
    auto segment_has_centromere = [](const dsb_end& a, const dsb_end& b) {
        // Different arms → must cross centromere
        if (a.chromosome_arm != b.chromosome_arm) return true;

        // Same arm but opposite orientation → crosses centromere
        if (a.upstream_downstream != b.upstream_downstream) return true;

        return false;
    };

    // Group ends by chromosome + arm
    unordered_map<int, unordered_map<int, vector<dsb_end>>> by_chrom_arm;

    for (const auto& e : all_dsb_ends) {
        by_chrom_arm[e.chromosome_id][e.chromosome_arm].push_back(e);
    }

    cout << "\n=== True Chromosome Segment Visualization ===\n";

    for (auto& chrom_pair : by_chrom_arm) {
        int chrom = chrom_pair.first;

        for (auto& arm_pair : chrom_pair.second) {
            int arm = arm_pair.first;
            auto& ends = arm_pair.second;

            // Extract unique breakpoints
            vector<double> breakpoints;
            for (auto& e : ends) breakpoints.push_back(e.position_in_chromosome);

            sort(breakpoints.begin(), breakpoints.end());
            breakpoints.erase(unique(breakpoints.begin(), breakpoints.end()), breakpoints.end());

            if (breakpoints.size() < 2) continue;
            ...
            for (size_t i = 0; i + 1 < breakpoints.size(); ++i) {
                ...
                // Find ends at start and end
                const dsb_end* A = nullptr;
                const dsb_end* B = nullptr;

                for (auto& e : ends) {
                    if (fabs(e.position_in_chromosome - start) < 1e-9) A = &e;
                    if (fabs(e.position_in_chromosome - end)   < 1e-9) B = &e;
                }

                bool has_cent = false;
                if (A && B) has_cent = segment_has_centromere(*A, *B);

                cout << "  Segment " << (i + 1) << ": "
                     << "Chromosome " << chrom
                     << ", Arm " << arm
                     << " [" << start << " - " << end << "]  "
                     << (has_cent ? "CENTROMERE" : "NO CENTROMERE")
                     << "\n";
            }
        }
    }
...
void repaired_chromosome::parse_cell_data(const std::string& cell_data) {
    ...
    for (size_t i = 0; i + 2 < lines.size(); i += 3) {
        ...
        all_dsb_ends.push_back(pair.end1);
        all_dsb_ends.push_back(pair.end2);
