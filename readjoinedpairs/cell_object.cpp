...
// Function to parse a DSB end string
...
class repaired_chromosome_geometry {
public:
    std::string cell_id;
    int dna_dsb_count;
    int unrepaired_dsb;
    ...
    repaired_chromosome_geometry(const std::pair<std::string, std::string>& cell) {
        // Parse repair data
...
int main() {
    ...
    for (size_t i = 0; i < cells.size(); ++i) {
        repaired_chromosome_geometry geom(cells[i]);
        cout << "Cell " << (i) << ":" << endl;
